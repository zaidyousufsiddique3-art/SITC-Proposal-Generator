...
const NotificationBanner: React.FC<{ message: string, type: 'success' | 'error' | 'info', onClose: () => void }> = ({ message, type, onClose }) => {
    ...
    return (
        <div className={`fixed top-4 right-4 z-[100] ${bgColors[type]} text-white px-6 py-4 rounded-lg shadow-2xl flex items-center gap-4 animate-fadeIn`}>
            <span className="font-bold">{message}</span>
            {type !== 'info' && <button onClick={onClose} className="text-white/80 hover:text-white font-bold">×</button>}
        </div>
    );
};
...
const App: React.FC = () => {
    ...
    // Auto-save effect
    useEffect(() => {
        if (viewMode === 'form' && formData.id && user) {
            autoSaveDraft(formData, () => {
                // Optional: show a small "Saved" indicator somewhere, or just silent
                console.log("Auto-saved draft");
            });
        }
    }, [formData, viewMode, user]);

    // Refresh data when entering dashboard to ensure new drafts appear
    useEffect(() => {
    ...
    const saveToStorage = async (proposal: ProposalData) => {
        try {
            ...
            showNotification("Error saving proposal: " + e.message, 'error');
        }
    };
    ...
    const handleCreateNew = async () => {
        try {
            if (!user) {
                console.error("User not found in handleCreateNew");
                return;
            }
            ...
            try {
                ...
                settings = { defaultCompanyLogo: '' }; // Fallback
            }
            ...
            console.error("Error creating new proposal:", e);
            showNotification("Failed to create new proposal: " + e.message, 'error');
        }
    };
    ...
    const handleDuplicate = async (proposal: ProposalData) => {
        ...
        try {
            ...
            await refreshData();
            showNotification("Proposal duplicated successfully");
        } catch (e: any) {
            showNotification("Error duplicating: " + e.message, 'error');
        }
    };
    ...
    const handleSaveProposal = async (isDraft: boolean) => {
        ...
        if (!formData.proposalName) {
            showNotification("Please enter a Proposal Name in the Branding section (Step 1).", 'error');
            setStep(0);
            return;
        }
        ...
        try {
            console.log("=== SAVE PROPOSAL START ===");
            console.log("isDraft:", isDraft);
            console.log("Proposal ID:", updatedProposal.id);
            console.log("Created By:", updatedProposal.createdBy);
            console.log("Company ID:", updatedProposal.companyId);

            if (isDraft) {
                console.log("Calling saveDraft...");
                await saveDraft(updatedProposal);
                console.log("saveDraft completed");
                showNotification("Draft saved successfully!");
            } else {
                console.log("Calling saveProposal...");
                await saveProposal(updatedProposal);
                console.log("saveProposal completed");
            ...
            console.log("Saved proposals count:", savedProposals.length);

            if (!isDraft) {
                setViewMode('preview');
            } else {
                // Navigate to dashboard after saving draft so user can see it
                console.log("Navigating to dashboard...");
                setViewMode('dashboard');
                setSubMode('my_proposals');
            }
            console.log("=== SAVE PROPOSAL END ===");
        } catch (e: any) {
            console.error("=== SAVE PROPOSAL ERROR ===", e);
    ...
    const handleCreateUser = async (role: 'admin' | 'user') => {
        ...
        if (passErr) {
            showNotification(passErr, 'error');
            return;
        }
        ...
        try {
            if (role === 'admin') {
                if (!newUser.companyId) {
                    showNotification("Please select a company for this Admin.", 'error');
                    return;
                }
            ...
            showNotification(role === 'admin' ? "Admin created successfully." : "User created successfully.");
        ...
        return (
            <div className="w-full max-w-7xl mx-auto p-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    {/* Header Branding */}
                    <div className="flex items-center gap-4">
                        <img src={SITCLogoImg} className="h-20 w-auto object-contain" alt="SITC Logo" />
                        {/* Removed "Portal" text as requested to rely on logo, or keep it? User said "add this logo... big enough...". 
                           The screenshot shows "Portal" next to the old logo. 
                           User said "remove the logo (in screenshot) and add this logo...". 
                           I will keep "Portal" text but make the logo prominent as requested. 
                           Actually, usually the logo contains the name. The new logo has text "Saudi International Travel Company".
                           So maybe remove "Portal" text or keep it small?
                           The user said "add this logo... big enough to be noticed".
                           I will render just the logo if it has text, or logo + Portal.
                           Let's keep "Portal" but maybe smaller or secondary.
                           Actually, let's just replace the old logo/text combo with this new logo.
                        */}
                    </div>
                            ...
                            {displayedProposals.map((p) => (
                                <div key={p.id} className="bg-slate-800/80 p-6 rounded-xl border border-slate-700 hover:border-ai-accent/50 transition shadow-lg flex flex-col relative group">
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        <div>
                                            <h3 className="text-xl font-bold text-white truncate w-48 flex items-center gap-2">
                                                {p.proposalName || p.customerName || 'Untitled'}
                                                {p.isDraft && <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-500 text-[10px] rounded uppercase tracking-wider border border-yellow-500/30">Draft</span>}
                                            </h3>
                ...
                {subMode === 'account_settings' && (
                    <div className="glass p-6 rounded-xl max-w-md mx-auto">
                        ...
                        <Button onClick={async () => {
                            try {
                                await changePassword(user.email, passData.current, passData.new);
            ...
            <div className="w-full max-w-4xl glass rounded-2xl shadow-2xl overflow-hidden border border-slate-700 flex flex-col h-[90vh] md:h-auto">
                ...
                <div className="p-4 md:p-6 border-b border-slate-700 flex justify-between items-center bg-slate-900/50">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Button onClick={() => handleSaveProposal(true)} disabled={isSaving} className="bg-green-600 text-white gap-2 h-8 text-sm"><SaveIcon /> {isSaving ? 'Saving...' : 'Save Draft'}</Button>
                    </div>
                </div>
                ...
                <div className="p-4 border-t border-slate-700 bg-slate-900/50 flex justify-between items-center">
                    ...
                    {step < Steps.length - 1 ? <Button onClick={() => setStep(s => Math.min(Steps.length - 1, s + 1))}>Next Step</Button> : <Button onClick={() => handleSaveProposal(false)} disabled={isSaving} className="bg-ai-accent text-slate-900">{isSaving ? 'Generating...' : 'Generate Proposal'}</Button>}
                </div>
            </div>
        </div>
    );
